...
async function verifyUpgrade(connection, database) {
  ...
  const [[hidden]] = await connection.execute('SELECT trang_thai FROM tu_vung WHERE id = ?', [wordId]);
...
async function verifyBehavior(t, { api, connection, admin }) {
  await t.test('word visibility filters new content while preserving learning and favorites', async () => {
    const credentials = { email: 'visibility@test.local', mat_khau: 'Test123456' };
    const user = await api('POST', '/api/auth/register', { ...credentials, ho_ten: 'Visibility learner' }, undefined, 201);
    const learner = await api('POST', '/api/auth/login', credentials);
    const topic = await api('POST', '/api/admin/topics', { ten: 'Visibility content' }, admin.accessToken, 201);
    const words = [];
    for (let index = 0; index < 8; index += 1) {
      words.push(await api('POST', '/api/admin/words', {
        chu_de_id: topic.id,
    ...
    const legacy = await api('POST', '/api/learning/start', { chu_de_id: topic.id, tong_so_tu: 50 }, token, 201);
    const quiz = await api('POST', '/api/quiz/start', { chu_de_id: topic.id, tong_so_tu: 50 }, token, 201);
    ...
    const result = (word) => api('POST', '/api/learning/result', {
    ...
    assert.equal((await api('GET', '/api/words/' + target.id, undefined, admin.accessToken)).trang_thai, 'inactive');
    await api('GET', '/api/words/' + target.id, undefined, token, 404);
    await api('GET', '/api/words/' + target.id, undefined, undefined, 404);

    const publicWords = await api('GET', '/api/words?topicId=' + topic.id + '&status=inactive', undefined, token);
    ...
    const hidden = await api('GET', '/api/admin/words?topicId=' + topic.id + '&status=inactive&limit=1', undefined, admin.accessToken);
    assert.equal(hidden.pagination.total, 2);
    assert.equal(hidden.items.length, 1);
    const active = await api('GET', '/api/admin/words?topicId=' + topic.id + '&status=active', undefined, admin.accessToken);
    assert.equal(active.pagination.total, 6);
    const all = await api('GET', '/api/admin/words?topicId=' + topic.id, undefined, admin.accessToken);
    assert.equal(all.pagination.total, 8);
    assert.equal((await api('GET', '/api/topics/' + topic.id)).word_count, 6);
    const adminTopic = (await api('GET', '/api/admin/topics', undefined, admin.accessToken)).find((item) => item.id === topic.id);
    assert.equal(adminTopic.word_count, 8);
    assert.equal(adminTopic.active_word_count, 6);
    assert.equal((await api('GET', '/api/progress/topics?topicId=' + topic.id, undefined, token))[0].total_words, 6);
    ...
    const [[favorite]] = await connection.execute('SELECT COUNT(*) AS count FROM yeu_thich WHERE nguoi_dung_id = ? AND tu_vung_id = ?', [user.id, target.id]);
    ...
    const fresh = await api('POST', '/api/learning/start', { chu_de_id: topic.id, tong_so_tu: 50 }, token, 201);
    assert.equal(fresh.danh_sach_tu.length, 6);
    assert.ok(fresh.danh_sach_tu.every((word) => word.id !== target.id && word.id !== words[7].id));
    const freshQuiz = await api('POST', '/api/quiz/start', { chu_de_id: topic.id, tong_so_tu: 50 }, token, 201);
    const [snapshots] = await connection.execute('SELECT tu_vung_id, noi_dung_trac_nghiem FROM phien_hoc_tu WHERE phien_hoc_tap_id = ?', [freshQuiz.phien_hoc_tap_id]);
    assert.equal(snapshots.length, 6);
    for (const member of snapshots) {
      assert.notEqual(member.tu_vung_id, target.id);
      const snapshot = typeof member.noi_dung_trac_nghiem === 'string' ? JSON.parse(member.noi_dung_trac_nghiem) : member.noi_dung_trac_nghiem;
    ...
    assert.deepEqual(await api('GET', '/api/quiz/' + quiz.phien_hoc_tap_id, undefined, token), quiz);
    const [[question]] = await connection.execute('SELECT dap_an_dung_id FROM cau_hoi_trac_nghiem WHERE id = ?', [quiz.cau_hoi.id]);
    await api('POST', '/api/quiz/' + quiz.phien_hoc_tap_id + '/answers', {
    ...
    await api('POST', '/api/learning/complete', { phien_hoc_tap_id: legacy.phien_hoc_tap_id }, token);
    ...
    assert.equal(progress.theo_chu_de.find((item) => item.topic_id === topic.id).learned_words, 6);
    ...
    await api('PUT', '/api/admin/topics/' + topic.id, { trang_thai: 'inactive' }, admin.accessToken);
    await api('GET', '/api/words/' + target.id, undefined, token, 404);
    await api('PUT', '/api/admin/topics/' + topic.id, { trang_thai: 'active' }, admin.accessToken);
    await setStatus('inactive');
    const another = words.find((word) => word.id !== target.id && word.trang_thai === 'active');
    const third = words.find((word) => word.id !== target.id && word.id !== another.id && word.trang_thai === 'active');
    for (const word of [another, third]) {
      await api('PUT', '/api/admin/words/' + word.id, { trang_thai: 'inactive' }, admin.accessToken);
    }
