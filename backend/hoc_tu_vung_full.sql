...
-- Bảng: Mã đặt lại mật khẩu dùng một lần
...
-- Bảng: Danh sách từ cố định trong từng phiên học
CREATE TABLE phien_hoc_tu (
    phien_hoc_tap_id CHAR(36) NOT NULL,
    tu_vung_id CHAR(36) NOT NULL,
    thu_tu INT NOT NULL DEFAULT 0,

    PRIMARY KEY (phien_hoc_tap_id, tu_vung_id),
    UNIQUE KEY uq_phien_thu_tu (phien_hoc_tap_id, thu_tu),

    FOREIGN KEY (phien_hoc_tap_id)
        REFERENCES phien_hoc_tap(id) ON DELETE CASCADE,
    FOREIGN KEY (tu_vung_id)
        REFERENCES tu_vung(id) ON DELETE CASCADE,
...
-- ==========================================
-- 11. DANH SÁCH TỪ TRONG PHIÊN HỌC
-- ==========================================
...
UNION ALL
SELECT '', 'Từ trong phiên học:', COUNT(*) FROM phien_hoc_tu;

SELECT '✅ Đã tạo database, bảng và dữ liệu mẫu thành công!' AS thong_bao;
